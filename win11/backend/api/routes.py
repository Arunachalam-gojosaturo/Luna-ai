            ...
            try:
                if platform.system() == "Windows":
                    if shutil.which("mpv"):
                        ...
                        # PowerShell SoundPlayer audio playback on Windows
                        ps_script = f"Add-Type -AssemblyName System.Windows.Forms; (New-Object System.Media.SoundPlayer '{tmp_name}').PlaySync()"
                        subprocess.run(["powershell", "-NoProfile", "-Command", ps_script], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
async def pick_folder():
    """Opens a native GUI folder picker dialog on Windows 11 and Linux."""
    import asyncio, sys, os, shutil, platform
    ...
    if platform.system() == "Windows":
        try:
            ps_code = "Add-Type -AssemblyName System.Windows.Forms; $f = New-Object System.Windows.Forms.FolderBrowserDialog; $f.Description = 'Select Project Folder'; if ($f.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Write-Output $f.SelectedPath }"
            ...
            print(f"[PickFolder Windows Exception]: {e}")

    # 1. Try zenity if available (GTK file chooser)
    if not selected_path and shutil.which("zenity"):
        try:
            proc = await asyncio.create_subprocess_exec(
    ...
    # 2. Try PyQt6 QFileDialog
    if not selected_path:
        try:
            home_dir = os.path.expanduser("~").replace("\\", "/")
            script = f"from PyQt6.QtWidgets import QApplication, QFileDialog; app = QApplication([]); res = QFileDialog.getExistingDirectory(None, 'Select Project Folder', '{home_dir}'); print(res)"
            proc = await asyncio.create_subprocess_exec(
...
async def open_file_manager(req: FolderActionRequest):
    """Detects and opens system File Explorer / File Manager."""
    ...
    if platform.system() == "Windows":
        try:
            if os.path.exists(target_path):
                ...
                return {"status": "success", "message": f"Opened {target_path} in Windows File Explorer"}
            else:
                subprocess.Popen(["explorer.exe", os.path.expanduser("~")])
                return {"status": "success", "message": "Opened User Home in Windows File Explorer"}
        except Exception as e:
            return {"status": "error", "message": str(e)}

    fm_candidates = ["dolphin", "thunar", "nautilus", "pcmanfm-qt", "pcmanfm", "nemo", "caja", "xdg-open"]
